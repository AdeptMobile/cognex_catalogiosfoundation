...
- (id) initWithContentGallery:(id<ContentViewBehavior>)aContentGallery {
    ...
    if (self) {
        ...
        NSString *pathTitle = [aContentGallery contentTitle];
...
- (GalleryDetailView *)contentView {
    return (GalleryDetailView *)self.view;
}

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    GalleryDetailView *detailView = [[GalleryDetailView alloc] initWithFrame:CGRectZero productNavController:(UINavigationController *)self andContentGallery:contentGallery];
...
- (void) viewWillAppear:(BOOL)animated {
    GalleryDetailView *detailView = (GalleryDetailView *)self.view;
    HierarchyNavigationViewController *hierarchyController = (HierarchyNavigationViewController *)detailView.hierarchyPopover.contentViewController;
    hierarchyController.navController = self.navigationController;
    
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadName:@"contentGalleryViewed" object:self userInfo:[NSDictionary dictionaryFromContentViewBehavior:contentGallery]];
    
    if (self.moviePlaying == NO) {
        [detailView addViewsOnAppear];
        // TODO: Set up the mail delegate from whatever is wrapping the ResourcePreviewviews
...
- (void) viewDidAppear:(BOOL)animated {
    ...
    [[[[self contentView] resourcePreviewView] docPreviewCollectionView] flashScrollIndicators];
...
- (MFMailComposeViewController *)setupEmailForContentItem:(ContentItem *)contentItem {
    // TODO Implement this
    GalleryDetailViewControllerConfig *config = [[SFAppConfig sharedInstance] getGalleryDetailViewConfig];
    
    MFMailComposeViewController *emailDialog = nil;
    if ([contentGallery hasResourceItems]) {
        for (id<ContentViewBehavior>resourceItem in [contentGallery contentResourceItems]) {
...
- (void) toolbarButtonTapped:(id)toolbarButton {
    switch ([toolbarButton tag]) {
        case 7:
        {
            GalleryDetailView *pdv = (GalleryDetailView *)self.view;
